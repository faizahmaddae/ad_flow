...
- **Init:** `await MobileAds.instance.initialize()` → `Future<InitializationStatus>` (completes on init or a 30s timeout; `adapterStatuses` reports mediation readiness). Configure via `MobileAds.instance.updateRequestConfiguration(RequestConfiguration(testDeviceIds:, tagForChildDirectedTreatment:, tagForUnderAgeOfConsent:, maxAdContentRating:))`. **Empirically, the documented 30s timeout is the native SDK's own promise, not a guarantee** — reproduced on a real Android emulator (system image with 0 Google accounts signed in): the underlying call never returned within 5.5 minutes, and `flutter run` never even got past the native launch splash (confirmed via `adb`: the app process sat at 0% CPU the whole time, `dumpsys activity` showed the native `Splash Screen` window still topmost, no ANR, no crash — a genuine indefinite native hang, not slowness). Root cause traced (via logcat) to Google Play Services' own Chimera/Dynamite module loader for `com.google.android.gms.ads.dynamite`, which runs automatically at Android process start — **before Flutter's engine or Dart isolate ever run**, so nothing on the Dart side (including a `.timeout()` on `AdSdk.initialize()`) can observe or route around it. This is a device/emulator Play-Services-state issue, not an `ad_flow` or plugin defect — see ADR-027. `ad_flow`'s own `AdFlow._start()` now still wraps `sdk.initialize()` in a 30s Dart-level timeout regardless, since a real (non-frozen) device can legitimately take longer than expected on a cold/flaky network and that case *is* observable from Dart.
