...
void main() {
  ...
  test('item 6: the FIRST genuine warm return shows an app-open ad (ADR-043)',
      () async {
    final c = controller();
    final m = manager(c);
    await boot(m);
    ...
    expect(
      ...
      1,
      reason: 'the first warm return of a session is a real impression, not a '
  ...
  test('item 6: a TRUE cold start still cannot show (no ad is loaded yet)',
      () async {
    final c = controller();
    final m = manager(c);
    sdk.loadHold = null;
    ...
    expect(
      sdk.appOpens,
      isEmpty,
      reason: 'invariant 3: nothing can show on a cold launch because nothing '
  ...
  test('item 4a: returning from a BANNER ad click shows no app-open (ADR-042)',
      () async {
    final c = controller();
    final m = manager(c);
    await boot(m);
  ...
  test('item 4b: no app-open while the app declares a blocking view ad on '
    ...
    expect(
      sdk.appOpens.single.showCalls,
      0,
      reason: 'an app-open ad must not cover content that is already showing '
