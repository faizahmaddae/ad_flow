...
- **Init:** `await MobileAds.instance.initialize()` → `Future<InitializationStatus>` (completes on init or a 30s timeout; `adapterStatuses` reports mediation readiness). Configure via `MobileAds.instance.updateRequestConfiguration(RequestConfiguration(testDeviceIds:, tagForChildDirectedTreatment:, tagForUnderAgeOfConsent:, maxAdContentRating:))`.
  - **⚠️ Threading gotcha — `initialize()` and `updateRequestConfiguration()` run on DIFFERENT native threads; never call the latter before the former completes.** Verified in the plugin's own Android source (`~/.pub-cache/.../google_mobile_ads-9.0.0/android/`): `MobileAds#initialize` is dispatched to a **background `new Thread(...)`** (`FlutterMobileAdsWrapper.initialize`), but `MobileAds#updateRequestConfiguration` is handled **synchronously on the platform (UI) thread** inside `GoogleMobileAdsPlugin.onMethodCall`, and it calls `MobileAds.getRequestConfiguration()` + `setRequestConfiguration()`. On a cold device those get/set calls force the Play Services Ads-Dynamite settings-manager singleton (`ChimeraMobileAdsSettingManagerCreatorImpl`) to bootstrap **on the platform thread**, racing `initialize()`'s background bootstrap of the *same* singleton — they deadlock the platform thread, which freezes the whole Flutter engine + Dart isolate (no first frame, no timer, no `.timeout()` fires; eventually the VM-service connection dies). Fix: **`await initialize()` first, THEN `updateRequestConfiguration()`.** This bit `ad_flow` hard (multi-round misdiagnosis as "native/environmental" — see ADR-027 + addenda — before the plugin-source read in ADR-028 found it). `updateRequestConfiguration` sends no ad request, so it must still run *unconditionally* (not gated on the consent result — finding #5), just ordered after init.
  - The documented 30s init timeout is the native SDK's own promise, not a hard guarantee — a cold/flaky network can legitimately exceed it, so `ad_flow`'s `AdFlow._start()` wraps `sdk.initialize()` in a 30s Dart-level timeout as belt-and-suspenders (ADR-027). (Earlier RESEARCH text here claimed an *unrecoverable* multi-minute native hang that no Dart code could touch; that was the misdiagnosed symptom of the ordering bug above, now fixed — see ADR-028.)
- **Full-screen ad EXPIRY (added 2026-07 audit):** Google's interstitial/rewarded/rewarded-interstitial guides state loaded ads **expire after about one hour** (app-open: 4 hours, already documented above) — a stale ad shown late may fail to display, or display but not count. There is no expiry callback; track the load time yourself. ad_flow enforces this via `maxAdAge` (default 55min for the three formats; `AppOpenConfig.expiry` for app-open) with a proactive replacement timer + a show-time check (ADR-051).
- **Plugin 9.0.0 upstream leak (added 2026-07 audit, verified in `ad_instance_manager.dart` `_invokeOnAdFailedToLoad`):** on a FAILED load the plugin auto-disposes interstitial/rewarded/rewarded-interstitial ads but **NOT `AppOpenAd`**, and the failure callback carries no ad reference — each failed app-open load permanently retains one plugin-side ad entry that a wrapper cannot release. Re-check on every plugin bump; consider filing/tracking upstream.
- **`AdWidget` cannot re-point its platform view (added 2026-07 audit, verified in `ad_containers.dart` `_AdWidgetState`):** it has no `didUpdateWidget`; the platform view captures the ad id at creation and Flutter only recreates a platform view when its `viewType` changes (never — one constant per plugin). Rebuilding an `AdWidget` with a DIFFERENT ad updates the element in place and keeps showing the old ad's (possibly disposed) view. Any handle swap must remount via a key (ADR-047).
- **Impression-level revenue:** every ad listener exposes **`onPaidEvent`** (`OnPaidEventCallback` → `PaidEvent` with value/currency/precision). Wire this for revenue analytics. `Ad.responseInfo` (available after load, refreshed per banner auto-refresh event) carries `responseId`/`mediationAdapterClassName`/`loadedAdapterResponseInfo` (`adSourceName`, `adSourceInstanceName`) — the mediation-visibility surface ad_flow summarizes as `AdResponseSummary`.
